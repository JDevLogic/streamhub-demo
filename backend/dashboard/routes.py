...
GET  /dashboard                    -- Main HTML dashboard (Basic Auth required)

GET  /dashboard/api/stats          -- Server stats JSON

GET  /dashboard/api/cache          -- Cache summary JSON

POST /dashboard/api/cache/clear    -- Clear a cache table

GET  /dashboard/api/intros         -- List all intro skip entries

POST /dashboard/api/intros         -- Create / update an intro skip entry (no_intro=true = episodio sin intro)

DELETE /dashboard/api/intros       -- Delete an intro skip entry

GET  /dashboard/api/intros/pending -- Episodes without intro skip configured

DELETE /dashboard/api/intros/pending -- Remove pending episode entry

GET  /dashboard/api/logs           -- Application logs (in-memory buffer)

POST /dashboard/api/logs/clear     -- Clear log buffer

GET  /dashboard/api/metrics        -- Per-endpoint performance metrics

GET  /dashboard/api/activity       -- Recent request activity log

GET  /dashboard/api/system         -- System information (disk, versions, etc.)
...
def _cache_counts() -> dict:
    ...
    return {
        ...
        "detail": _redis_count("detail"),

        "episodes": _redis_count("episodes"),

        "sources": _redis_count("sources"),
...
async def api_intros_pending(_user=Depends(_require_auth)):
    ...
    """Episodes pending intro config: already-seen without skip + next 5 predicted per content.
    ...
    for r in user_state_rows:
        ...
        for it in items:
            ...
            content_url = str(it.get("animeUrl") or "").strip().rstrip("/")

            if not content_url:

                continue

            short = content_url.rsplit("/", 1)[-1]
    ...
    latest_per_content: dict[str, tuple[int, float]] = {}
    ...
    for r in all_seen_rows:
        ...
        prev = latest_per_content.get(slug)

        if prev is None or ep > prev[0]:

            latest_per_content[slug] = (ep, r["last_seen"])
    ...
    slugs = list(latest_per_content.keys())

    sample_urls = [f"{slug}-{latest_per_content[slug][0]}" for slug in slugs]
    ...
    for slug, (ep, last_seen) in latest_per_content.items():

        max_ep = counts_by_slug.get(slug)

        if max_ep is None:

            # Unknown total -- skip predictions to avoid listing non-existent episodes.
...
async def api_source_history(
    ...
    Answers "when do failures happen" -- `by_hour` has 24 fixed buckets in
...
# -- Public read-only endpoint (no auth) -- used by the Flutter app --------------
