"""Persistent metrics: response times per endpoint.

Backed by Redis lists (falls back to in-memory if Redis is unavailable).

Redis keys:
  metrics:hits         -- hash  { endpoint: count }  (kept for compatibility)
  metrics:misses       -- hash  { endpoint: count }  (kept for compatibility)
...
def snapshot() -> dict:
    ...
    if _rc.is_available():
        ...
        while True:
            ...
            for k in keys:
                all_endpoints.add(k.removeprefix(_TIMES_PREFIX))
            if cursor == 0:
                break
        ...
        for ep in sorted(all_endpoints):
            ...
            result[ep] = {
                ...
                "misses":   m,
                "requests": len(ts),
                "hit_rate": round(h / cache_total, 3) if cache_total else None,
        ...
        with _lock:
            result = {}
            all_eps = sorted(set(_fb_hits) | set(_fb_misses) | set(_fb_times))
            for ep in all_eps:
                h = _fb_hits[ep]
                m = _fb_misses[ep]
                cache_total = h + m
